...
export default function App() {
      ...
      clips: [
        {
          ...
          volume: 1.0,
          speed: 1.0,
          transform: { x: 0, y: -20, scale: 1.2, rotation: 0, opacity: 0.9, zIndex: 2 },
          chromaKey: { enabled: false, color: '#00ff00', distance: 0.3, smoothness: 0.1, spill: 0.1 },
          keyframes: {},
      ...
      clips: [
        {
          ...
          sourceStart: 0,
          sourceDuration: 60,
          sourceUrl: STOCK_ASSETS[5].url,
          volume: 0.8,
          speed: 1.0,
          transform: { x: 0, y: 0, scale: 1.0, rotation: 0, opacity: 1, zIndex: 0 },
          chromaKey: { enabled: false, color: '#00ff00', distance: 0.3, smoothness: 0.1, spill: 0.1 },
          keyframes: {},
          effectType: 'none',
          effectIntensity: 0,
  ...
  const [savedProjects, setSavedProjects] = useState<SavedProject[]>([
    {
      id: 'proj-1',
      title: 'إعلان قصير Tik Tok',
      lastEdited: 'منذ ساعتين',
      durationSeconds: 12,
      clipCount: 4,
      thumbnailUrl: STOCK_ASSETS[0].thumbnail || '',
      tracks,
      aspectRatio: '9:16',
      enhancements,
    },
    {
      id: 'proj-2',
      title: 'فيديو ريلز احترافي',
      lastEdited: 'أمس',
      durationSeconds: 15,
      clipCount: 3,
      thumbnailUrl: STOCK_ASSETS[1].thumbnail || '',
      tracks,
      aspectRatio: '9:16',
      enhancements,
    },
  ...
  const [exportHistory, setExportHistory] = useState<ExportHistoryRecord[]>([
    {
      id: 'exp-1',
      projectTitle: 'إعلان قصير Tik Tok',
  ...
  // Helper calculation for total timeline duration
  const totalDuration = Math.max(
    15,
    ...tracks.flatMap((t) => t.clips.map((c) => c.startInTimeline + c.duration))
  );

  // Undo / Redo mock stacks
  const canUndo = true;
  const canRedo = false;
    ...
    >
      {currentScreen === 'home' ? (
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden">
          ...
          {/* Device Frame Option Toggle Bar */}
          ...
          <div className="flex-1 flex overflow-hidden relative">
            ...
            <Sidebar
              ...
              onAddStockAssetToTimeline={(asset) => {
                const newClip: Clip = {
                  id: 'clip-' + Date.now(),
                  trackId: asset.type === 'audio' ? 'track-a1' : 'track-v1',
                  name: asset.title,
                  type: asset.type,
                  startInTimeline: currentTime,
                ...
                setTracks((prev) =>
                  prev.map((t) =>
                    t.id === (asset.type === 'audio' ? 'track-a1' : 'track-v1')
            ...
            >
              {isMobileFrame ? (
                /* Sleek Smartphone Container Mockup */
            ...
            {/* Right Enhancement / Tool Inspector */}
